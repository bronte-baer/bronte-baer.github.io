...
author: "Bronte Baer, Danny Collins, Amy Ho, Victor Inyang"
date: "March 14, 2022"
output:
  pdf_document:
    toc: yes
...
Model 3 included all the variables in Model 2 with the addition of two categorical variables that are out of a creator's control, `category` and `staff_pick`. The inclusion of these variables increased the $R^2$ value to `r round(100*model3_r2,1)`%. For the ‘staff_pick’ variable, the default was `False`, and the model showed that if a campaign was marked as a staff pick its chance for success increased by 25%. The model did not show much change in the log-transformed `goal` coefficient from Model 2, and it was still statistically significant. There was a slight change in the `deadline_on_weekday` effect from `r round(100*model2_weekday_coef,1)`+/-`r round(100*model2_weekday_se,1)`% to `r round(100*model3_weekday_coef,1)`+/-`r round(100*model3_weekday_se,1)`%. The `deadline_season` variables' impacts changed slightly from a campaign with a `spring` deadline being `r round(100*model3_spring_coef,1)`+/-`r round(100*model3_spring_se,1)`% less likely to reach its funding goal, compared to `fall`, and `winter` decreasing the chance of success by `r round(100*model3_winter_coef,1)`+/-`r round(100*model3_winter_se,1)`%. The `normalized_campaign_length` variable's negative impact on campaign success decreased because it went from  `r round(100*model2_length_coef,1)`% less likely to reach the funding goal to  `r round(100*model3_length_coef,1)`% less likely.

* Categories that lower success chance (significantly), compared to art:
    * crafts (`r round(100*model3_crafts_coef,1)`+/-`r round(100*model3_crafts_se,1)`%)
    * film (`r round(100*model3_film_coef,1)`+/-`r round(100*model3_film_se,1)`%)
    * food (`r round(100*model3_food_coef,1)`+/-`r round(100*model3_food_se,1)`%)
    * journalism (`r round(100*model3_journalism_coef,1)`+/-`r round(100*model3_journalism_se,1)`%)
    * technology (`r round(100*model3_technology_coef,1)`+/-`r round(100*model3_technology_se,1)`%)
    * theater (`r round(100*model3_theater_coef,1)`+/-`r round(100*model3_theater_se,1)`%)
* Categories that increase success chance (significantly), compared to art:
    * comics (`r round(100*model3_comics_coef,1)`+/-`r round(100*model3_comics_se,1)`%)
    * design (`r round(100*model3_design_coef,1)`+/-`r round(100*model3_design_se,1)`%)
    * games (`r round(100*model3_games_coef,1)`+/-`r round(100*model3_games_se,1)`%)
    * photography (`r round(100*model3_photography_coef,1)`+/-`r round(100*model3_photography_se,1)`%)
...
#  Category = c("Comics", "Crafts", "Design", "Fashion", "Games",
#                 "Music", "Publishing", "Technology", "Theater"),
#  delta_v_art_base = c('-20%', 'N/S', '-20%', '+32%', '-23%', '-38%', '-20%',
#                       'N/S', 'N/S'),
#  delta_v_art_oom = c('+10%', '-15%', '+11%', '-10%', '+11%', '+10%', '+8%',
#                      '-9%', '-9%'),
#  effective_base = c('96%', '66%', '98%', '108%', '95%', '78%', '92%', 'N/S',
#                     'N/S'),
#  effective_oom = c('-7%', '-32%', '-6%', '-27%', '-6%', '-7%', '-9%', '-26%',
#                    '-26%')
#)
